...
function inicia_controle_Callback(hObject, eventdata, handles)
    ...
    if(cntdo==1);
        ...
        while(cntdo==1 && sai==0)
            ...
            OpenUltrasonic(SENSOR_4, '', h); %parametros: sensor, string vazia p/ n definir modo de operação e handle
...
function finaliza_controle_Callback(hObject, eventdata, handles)
    ...
    if(cntdo==1)
        StopMotor('all', 'off', h);
        %StopMotor(MOTOR_A, 'off', h); desativei para poder melhorar o
        %desempenho e facilitar o codigo e testar a funçao com 'all'
