...
%Teste sobre as decisoes de mudança de direção

%RESULTADOS:
%novo modelo de decisão de direção aprovado
...
%function dist_dir_esq
    ...
    if(distdir > distesq)
            if (distdir > 25)
                disp('vira direita 1');
                mC=NXTMotor('C', 'Power',30, 'TachoLimit',1100);
                mB=NXTMotor('B', 'Power',-30, 'TachoLimit',1100);
                mB.SendToNXT(h);
                mC.SendToNXT(h);
                mB.WaitFor(10, h);
                mC.WaitFor(10, h);
